...
for sample in samples:
    ...
    r1_zero_prompt = (
        r1_zero_template.format(
            question=question
        )
    )

    three_shot_prompt = (
        three_shot_template.format(
            question=question
        )
    )
    ...
    r1_zero_prompts.append(
        r1_zero_prompt
    )

    three_shot_prompts.append(
        three_shot_prompt
    )
...
def evaluate_responses(
    ...
    for response, ground_truth in zip(
        ...
        if (
            format_reward == 1.0
            and answer_reward == 1.0
        ):
            stats["correct_and_formatted"] += 1

        elif (
            format_reward == 1.0
            and answer_reward == 0.0
        ):
...
for i in range(len(samples)):
    ...
    print(question_only_responses[i])

    print("\nR1 zero response:")
    print(r1_zero_responses[i])

    print("\nThree-shot response:")
    print(three_shot_responses[i])
